...
void experiment_single_table_mb(CSketch_M<stype, dtype>& csketch, dtype* data, std::pair<stype, dtype>* &freq_decreasing) {
  ...
  double *rtimes = new double[no_max_threads + 1];
  double *wtimes = new double[no_max_threads + 1];
...
#pragma omp for schedule(static, 1)  
    ...
    wtimes[nt] = omp_get_wtime()- t1 - rtimes[nt];
    //std::cout << omp_get_wtime();
    times[nt] = wtimes[nt] + rtimes[nt];
    std::cout << "\tNo thr.: " << nt << "\ttime: " << times[nt] << "\tthrough.: " << no_stream / times[nt] << "\tRtime: " << rtimes[nt] << "\tWtime: " << wtimes[nt];
#ifdef CERROR
