...
#pragma omp parallel for schedule(static)
  ...
  D(
    ofstream out;
    out.open("z3_nebulas_big_real.txt");
    std::cout << "Printing Real Frequencies" << std::endl;
    //std::cout << "Top frequencies: " << std::endl;
    for(unsigned i = 0; i < TOP; i++) {
      //std::cout << i << "\t" << freq_decreasing[i].second << "\t" << freq_decreasing[i].first << std::endl;
      //std::cout << freq_decreasing[i].first << std::endl;
      out << freq_decreasing[i].first << std::endl;
    }
    out.close();
    std::cout << std::endl;
  );
