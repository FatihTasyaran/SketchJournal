...
test1=pd.DataFrame(lis1,columns=["1 Thread"])
...
for row in file16:
    lis16.append(row[0:-1])

for row in file16bar:
    lis16barrier.append(row[0:-1])
    
test16=pd.DataFrame(lis16,columns=["16 Thread"])
test16.reset_index(drop=True, inplace=True)

test16barrier=pd.DataFrame(lis16barrier,columns=["16 Thread Barrier"])
test16barrier.reset_index(drop=True, inplace=True)

    
q=pd.concat([testreal,test1,test1barrier,test2,test2barrier,test4,test4barrier,test8,test8barrier,test16,test16barrier],axis=1)
