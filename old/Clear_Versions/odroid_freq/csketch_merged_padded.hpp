...
public:
  ...
  virtual double printFreq(std::pair<stype, dtype>* &freq_decreasing, unsigned hitterRankThreshold, TabularHashMerged *hashy, unsigned numt) = 0;
  virtual double printFreq2(std::pair<stype, dtype>* &freq_decreasing, unsigned hitterRankThreshold, TabularHashMerged *hasht, unsigned numt) = 0;
...
/*  virtual uint32_t hashElement(const d_type &data) {
  ...
  double printFreq(std::pair<s_type, d_type>* &freq_decreasing, unsigned hitterRankThreshold, TabularHashMerged *hashy, unsigned numt) {
    ...
    std::ofstream out;
    out.open("odroid_small"+std::to_string(numt) + ".txt");
    for(int i = 0; i < hitterRankThreshold; i++) {
      d_type curr = freq_decreasing[i].second;
  ...
  double printFreq2(std::pair<s_type, d_type>* &freq_decreasing, unsigned hitterRankThreshold, TabularHashMerged *hashy, unsigned numt) {
    ...
    std::cout << "Printing " << numt << " frequencies" << std::endl;
    std::ofstream out;
    out.open("odroid_small_halfbarrier"+std::to_string(numt) + ".txt");
    for(int i = 0; i < hitterRankThreshold; i++) {
      d_type curr = freq_decreasing[i].second;
  ...
  double printFreq3(std::pair<s_type, d_type>* &freq_decreasing, unsigned hitterRankThreshold, TabularHashMerged *hashy, unsigned numt) {
    std::cout << numt << " Threads" << std::endl;
    std::cout << "Printing " << numt << " frequencies" << std::endl;
    std::ofstream out;
    out.open("odroid_small_fullbarrier"+std::to_string(numt) + ".txt");
    for(int i = 0; i < hitterRankThreshold; i++) {
      d_type curr = freq_decreasing[i].second;
