...
zipf_val = sys.argv[8]
repeat   = sys.argv[9] ##Zipfian Distribution
...
time_array_padding_barrier = []
through_array_padding_barrier = []
err_array_padding_barrier = []

hash_array_padding_barrier = []
write_array_padding_barrier = []
...
for c in range(0, len(col_sizes)):
    ...
    for re in range(repeat): ## FROM NOW ON, REPEAT TIME
        ...
        time_array_padding.append(time_array_cont)
        hash_array_padding.append(hash_array_cont)
        write_array_padding.append(write_array_cont)
        through_array_padding.append(through_array_cont)
        err_array_padding.append(err_array_cont)
        ...
        time_array_padding_barrier.append(time_array_cont)
        hash_array_padding_barrier.append(hash_array_cont)
        write_array_padding_barrier.append(write_array_cont)
        through_array_padding_barrier.append(through_array_cont)
        err_array_padding_barrier.append(err_array_cont)
    ...
    appendName('PADDING')
    appendName('TIME')
    appendListofOneCOLSIZE(time_array_padding)
    appendName('HASH TIME')
    appendListofOneCOLSIZE(hash_array_padding)
    appendName('WRITE TIME')
    appendListofOneCOLSIZE(write_array_padding)
    appendName('ERROR')
    appendListofOneCOLSIZE(err_array_padding)
    appendName('THROUGHPUT')
    appendListofOneCOLSIZE(through_array_padding)

    appendName('PADDING WITH BARRIER')
    appendName('TIME')
    appendListofOneCOLSIZE(time_array_padding_barrier)
    appendName('HASH TIME')
    appendListofOneCOLSIZE(hash_array_padding_barrier)
    appendName('WRITE TIME')
    appendListofOneCOLSIZE(write_array_padding_barrier)
    appendName('ERROR')
    appendListofOneCOLSIZE(err_array_padding_barrier)
    appendName('THROUGHPUT')
    appendListofOneCOLSIZE(through_array_padding_barrier)
