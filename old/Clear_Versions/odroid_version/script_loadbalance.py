...
time_array_lb_padding = []
through_array_lb_padding = []
err_array_lb_padding = []

hash_array_lb_padding = []
write_array_lb_padding = []
...
for c in range(0, len(col_sizes)):
    ...
    for re in range(repeat): ## FROM NOW ON, REPEAT TIME
        ...
        time_array_lb_padding.append(time_array_cont)
        hash_array_lb_padding.append(hash_array_cont)
        write_array_lb_padding.append(write_array_cont)
        through_array_lb_padding.append(through_array_cont)
        err_array_lb_padding.append(err_array_cont)
    ...
    appendName('AFFINITY W/BATCH&PADDING')
    appendName('TIME')
    appendListofOneCOLSIZE(time_array_lb_padding)
    appendName('HASH TIME')
    appendListofOneCOLSIZE(hash_array_lb_padding)
    appendName('WRITE TIME')
    appendListofOneCOLSIZE(write_array_lb_padding)
    appendName('ERROR')
    appendListofOneCOLSIZE(err_array_lb_padding)
    appendName('THROUGHPUT')
    appendListofOneCOLSIZE(through_array_lb_padding)
