...
\institute{Sabanc{\i} University, Istanbul, Turkey, \\ \email{\scriptsize{\{fatihtasyaran,keremyildirir,mkemaltas,kaya\}@sabanciuniv.edu}}}
...
\renewcommand{\baselinestretch}{0.9}
 \begin{algorithm}[H]
	 ...
	 \KwOut{ ${\tt res}[4]$: filled with hash values }
	 	$mask \leftarrow 0x000000ff$ \\
		$x \leftarrow data$ \\
		$c \leftarrow 4*(x \& mask)$ \\
		\For{$i\leftarrow 0$ \KwTo $4$}{
			${\tt res[i]}  \leftarrow  {\tt tbl}[0][c+i]$\\
	}
	$x \leftarrow x>>8$ \\
			\For{$i\leftarrow 1$ \KwTo $4$}{
				$c \leftarrow 4*(x \& mask)$ \\
				${\tt res[0]}  \leftarrow {\tt res[0]} \oplus {\tt tbl}[i][c]$\\
				${\tt res[1]}  \leftarrow {\tt res[1]} \oplus  {\tt tbl}[i][c+1]$\\
				${\tt res[2]}  \leftarrow {\tt res[2]} \oplus  {\tt tbl}[i][c+2]$\\
				${\tt res[3]}  \leftarrow {\tt res[3]} \oplus  {\tt tbl}[i][c+3]$\\
				$x \leftarrow x>>8$ \\ 
			}
		%}
\label{algo:tabhash} 	
...
{\bf Xeon} &   17.6 & 60.0 & 22.6 & 17.8  &  {\bf Xeon} & 17.9 & 57.6 & 22.6 & 12.9 \\
{\bf Pi} & 1.3 & 3.9 & 1.3 & 3.3 &{\bf Pi} & 1.3 & 4.1 & 1.2 & 3.2 \\
{\bf Odroid} &   1.6 & 9.0 & 1.6 & 6.6 &  {\bf Odroid} & 1.6 & 9.0 & 1.7 & 6.1  \\
\end{tabular}
       }
